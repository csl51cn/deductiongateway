...
public class BatchConfig {
    ...
    public JdbcCursorItemReader<AutoBatchDeduction> reader(@Qualifier("remoteDataSource") DataSource dataSource) {
        ...
        String sql = "SELECT distinct  Date_Id,业务编号,合同编号,计划期数,计划还款日,还款账号银行,代扣卡折类型,还款账号,还款账户名,代扣人证件类型,代扣人证件号码,BX_Plan,Fwf_Plan,服务费管理司,LoginId  FROM Temp_当前代扣数据 WHERE CONVERT (VARCHAR, 计划还款日, 1) = CONVERT (VARCHAR, GETDATE(), 1) and LoginId = 14";
